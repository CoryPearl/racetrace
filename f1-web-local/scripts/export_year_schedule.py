...
Writes web/public/data/schedule/{year}.json — same shape as GET /api/schedule/{year}.

Run from repo root:
  PYTHONPATH=. python3 scripts/export_year_schedule.py --year 2024
  PYTHONPATH=. python3 scripts/export_year_schedule.py --year 2024 --write-default-year
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--write-default-year",
        action="store_true",
        help="Also write web/public/data/default-year.json for static-only hosting",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    sched_dir = ROOT / "web" / "public" / "data" / "schedule"
    ...
    if args.write_default_year:
        default_path = ROOT / "web" / "public" / "data" / "default-year.json"
