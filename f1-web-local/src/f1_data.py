...
from src.lib.tyres import TYRE_EXPECTED_STINT_LAPS, get_tyre_compound_int
...
def _f1_project_root() -> str:
    """Repo root (parent of ``src/``). Used so cache paths work even when cwd ≠ project root."""
...
def _safe_computed_relative(relative_path: str) -> str:
    rel = relative_path.replace(os.sep, "/").lstrip("/")
    if not rel or any(part == ".." for part in rel.split("/")):
        raise ValueError(f"invalid computed_data relative path: {relative_path!r}")
    return rel
...
def load_computed_pickle(relative_path: str):
    ...
    xz_name = f"{rel}.xz"
    for croot in _compressed_computed_data_dirs():
        xz_path = os.path.join(croot, xz_name)
        if os.path.isfile(xz_path):
            print(f"Loaded LZMA-compressed cache (streaming): {xz_path}")
            with lzma.open(xz_path, "rb") as f:
                return pickle.load(f)
    tried = [plain] + [os.path.join(cr, xz_name) for cr in _compressed_computed_data_dirs()]
    raise FileNotFoundError(
        f"No pickle or .xz for {rel!r}; tried: {tried!r} (project root {_f1_project_root()!r})"
    )


def _computed_telemetry_pkl_candidates(session, cache_suffix: str) -> list[str]:
    """
    Possible relative paths for ``{name}_{cache_suffix}_telemetry.pkl`` on disk.

    FastF1 uses ``str(session)`` (see Session.__repr__) which contains spaces and **colons**
    (``Round 1:``). Compressed archives or other OSes sometimes rename files (e.g. ``:`` → ``_``),
    so we also try stable aliases built from ``event.EventName`` and round number.
    """
    seen: set[str] = set()
    out: list[str] = []
    ...
    add(f"{str(session).replace(' ', '_')}_{cache_suffix}_telemetry.pkl")
    s2 = str(session).replace(" ", "_").replace(":", "_")
    if s2 != str(session).replace(" ", "_"):
        add(f"{s2}_{cache_suffix}_telemetry.pkl")

    try:
        ev = session.event
        en = str(ev.EventName).replace(" ", "_")
        y = int(ev.year)
        r = int(ev.RoundNumber)
        sn = str(session.name).replace(" ", "_")
...
def load_computed_pickle_first(candidates: list[str]):
    """Try several relative paths (plain pickle or compressed sibling)."""
    last: FileNotFoundError | None = None
    for rel in candidates:
        try:
            return load_computed_pickle(rel)
        except FileNotFoundError as e:
            last = e
            continue
    if last is not None:
        raise FileNotFoundError(
            f"No pickle or .xz for any of {len(candidates)} candidate name(s): {candidates!r}"
        ) from last
    raise FileNotFoundError("No candidate paths to load")


def _finite_float(v, default=0.0):
    """JSON-safe float: NaN/Inf from interpolation or gaps become default."""
    try:
        x = float(v)
...
def _finite_int_round(v, default=0):
    """JSON-safe int for lap/gear/drs; NaN/Inf from telemetry gaps become default."""
    try:
        x = float(v)
    except (TypeError, ValueError):
        return default
...
def _resample_nearest_1d(timeline, t_sorted, values):
    """Pick nearest source sample per timeline step (for discrete channels like DRS)."""
    if len(t_sorted) == 0:
...
def _speed_kph_to_mps(kph: float) -> float:
    """FastF1 Speed column is km/h."""
    return max(0.0, float(kph)) / 3.6


def _starting_grid_rank(session) -> dict:
    """
    Abbreviation -> grid slot (1 = pole). From session.results; empty if missing.
    Used so lap-1 / formation playback matches the starting grid instead of
    noisy distance-based ordering vs. car positions on track.
    """
    out = {}
    try:
...
def _gap_seconds_behind(
    distance_ahead_m: float, distance_behind_m: float, speed_kph_behind: float
) -> float:
    """
    Approximate time gap in seconds: track distance to the car ahead divided by
    this car's speed (m/s). Same idea as TV interval timing.
    """
    delta_m = max(0.0, float(distance_ahead_m) - float(distance_behind_m))
    mps = _speed_kph_to_mps(speed_kph_behind) if speed_kph_behind is not None else 0.0
    v = max(1.0, mps)
    return float(delta_m) / v
...
def _process_single_driver(args):
    ...
    for _, lap in laps_driver.iterlaps():
        ...
        tyre_compund_as_int = get_tyre_compound_int(compound)
        tyre_life = lap.TyreLife if pd.notna(lap.TyreLife) else 0
        tyre_life_val = lap.TyreLife if pd.notna(lap.TyreLife) else None

        compound_changed = (
            prev_compound is not None
            and not pd.isna(compound)
            and not pd.isna(prev_compound)
            and str(compound) != str(prev_compound)
        )
        tyre_reset = (
            prev_tyre_life is not None
            and tyre_life_val is not None
            and float(tyre_life_val) < float(prev_tyre_life)
        )

        # Anchor stint on first lap we actually have telemetry, so empty early laps
        # (common) do not pin stint_start to lap 1 while data starts at lap 5 → "4".
        if stint_start_lap is None:
            if not lap_tel.empty:
                stint_start_lap = lap_number
        elif compound_changed or tyre_reset:
            stint_start_lap = lap_number

        prev_compound = compound
        if tyre_life_val is not None:
            prev_tyre_life = float(tyre_life_val)

        # FastF1 TyreLife = laps on this set (1 = first lap out of the pits). User UI is 0-based.
        if tyre_life_val is not None:
            laps_since_pit = max(0, int(float(tyre_life_val)) - 1)
        elif stint_start_lap is not None:
            laps_since_pit = max(0, lap_number - stint_start_lap)
        else:
            laps_since_pit = 0
        ...
        tyre_laps_since_pit_all.append(np.full_like(t_lap, float(laps_since_pit)))
        ...
        # d_lap is metres along this lap only; without this, gap-to-leader uses
        # per-lap distance and clamps to 0 whenever the leader is on the next lap.
        if d_lap.size > 0:
            lap_end = float(np.nanmax(d_lap))
            if np.isfinite(lap_end) and lap_end > 0:
                total_dist_so_far += lap_end
    ...
    tyre_compounds, tyre_life_all, tyre_laps_since_pit_all, speed_all, gear_all, drs_all = [
    ...
    return {
        ...
        "data": {
            ...
            "tyre_laps_since_pit": tyre_laps_since_pit_all,
            "speed": speed_all,
            "gear": gear_all,
            "drs": drs_all,
...
def load_session(year, round_number, session_type="R"):
    ...
    # the exception (soft_exceptions) — session.load still completes but _laps
    # is unset, which later raises DataNotLoadedError on session.laps.
...
def _fork_pool_map(map_fn, tasks):
    """
    map_fn applied to each task. Uses a fork-based process pool only when safe:
    default multiprocessing uses "spawn" on macOS; workers unpickle Session
    without loaded data → FastF1 "not been loaded yet". Fork inherits memory
    but is unsafe with extra threads (e.g. uvicorn), so those use sequential.
    """
    n = len(tasks)
    if n == 0:
        return []
    if n == 1:
        return [map_fn(tasks[0])]

    num_workers = min(cpu_count(), n)
    if num_workers <= 1:
        return [map_fn(t) for t in tasks]

    if os.environ.get("F1_TELEMETRY_NO_POOL", "").lower() in ("1", "true", "yes"):
        return [map_fn(t) for t in tasks]

    if os.name == "nt" or threading.active_count() != 1:
        if threading.active_count() != 1:
            print(
                "Processing tasks sequentially "
                "(multi-threaded process: parallel pool would break FastF1 Session)."
            )
        return [map_fn(t) for t in tasks]
    ...
    try:
        with ctx.Pool(processes=num_workers) as pool:
            return pool.map(map_fn, tasks)
    except Exception as e:
        print(f"Process pool failed ({e}); falling back to sequential.")
        return [map_fn(t) for t in tasks]
...
def get_race_telemetry(session, session_type="R", force_refresh: bool = False):
    ...
    try:
        if not force_refresh and "--refresh-data" not in sys.argv:
            frames = load_computed_pickle_first(
    ...
    results = _fork_pool_map(_process_single_driver, driver_args)
    ...
    for code, data in driver_data.items():
        ...
        arrays_to_resample = [
            ...
            data["tyre_laps_since_pit"][order],
            data["speed"][order],
            data["gear"][order],
            data["drs"][order],
            data["throttle"][order],
        ...
        resampled = []
        for i, arr in enumerate(arrays_to_resample):
            # 7 = laps since pit (integer per lap); 10 = DRS discrete
            if i in (7, 10):
                resampled.append(_resample_nearest_1d(timeline, t_sorted, arr))
            else:
                resampled.append(np.interp(timeline, t_sorted, arr))
        (
            x_resampled,
            y_resampled,
            dist_resampled,
        ...
        resampled_data[code] = {
            ...
            "tyre_laps_since_pit": tyre_laps_since_pit_resampled,
    ...
    grid_rank = _starting_grid_rank(session) if session_type in ("R", "S") else {}
    ...
    for i in range(num_frames):
        ...
        for code in driver_codes:
            ...
            tl = _finite_float(d["tyre_life"][i])
            tsp = _finite_float(d["tyre_laps_since_pit"][i])
            snapshot.append({
                "code": code,
                "dist": _finite_float(d["dist"][i]),
                "x": _finite_float(d["x"][i]),
                "y": _finite_float(d["y"][i]),
                "lap": _finite_int_round(d["lap"][i]),
                "rel_dist": _finite_float(d["rel_dist"][i]),
                "tyre": _finite_float(d["tyre"][i], -1.0),
                "tyre_life": tl,
                "tyre_laps_since_pit": max(0, _finite_int_round(tsp)),
                "speed": _finite_float(d["speed"][i]),
                "gear": _finite_int_round(d["gear"][i]),
                "drs": _finite_int_round(d["drs"][i]),
                "throttle": _finite_float(d["throttle"][i]),
                "brake": _finite_float(d["brake"][i]),
            })
        ...
        # 5b. Race order: by distance once racing; on lap 1 (formation / start)
        # use official grid so P1 = pole matches the cars on track.
        max_lap_here = max(s.get("lap", 0) for s in snapshot)
        if grid_rank and max_lap_here <= 1:
            snapshot.sort(
                key=lambda r: (grid_rank.get(r["code"], 999), str(r["code"])),
            )
        else:
            # Leader = largest race distance covered
            snapshot.sort(
                key=lambda r: (r.get("lap", 0), r["dist"]), reverse=True,
            )

        leader = snapshot[0]
        leader_lap = leader["lap"]

        # 5c. Gaps in seconds (distance delta / trailing car speed) for leaderboard UI
        frame_data = {}
        ...
        for idx, car in enumerate(snapshot):
            ...
            if idx == 0:
                ...
                gap_leader_s = _gap_seconds_behind(leader["dist"], car["dist"], spd)
                ahead = snapshot[idx - 1]
                gap_ahead_s = _gap_seconds_behind(ahead["dist"], car["dist"], spd)
            ...
            frame_data[code] = {
                ...
                "tyre_laps_since_pit": car["tyre_laps_since_pit"],
                "position": position,
                "speed": car["speed"],
                "gear": car["gear"],
                "drs": car["drs"],
                ...
                "brake": car["brake"],
                "gap_leader_s": round(gap_leader_s, 3),
                "gap_ahead_s": None
                if gap_ahead_s is None
                else round(gap_ahead_s, 3),
            }
    ...
    cdir = _computed_data_dir()
    if not os.path.exists(cdir):
        os.makedirs(cdir)

    # Save using pickle (10-100x faster than JSON)
    tyre_expected_laps = {
        str(k): int(v) for k, v in TYRE_EXPECTED_STINT_LAPS.items()
    }
    with open(os.path.join(cdir, f"{event_name}_{cache_suffix}_telemetry.pkl"), "wb") as f:
        pickle.dump({
            "frames": frames,
            ...
            "tyre_expected_laps": tyre_expected_laps,
...
def get_driver_quali_telemetry(session, driver_code: str, quali_segment: str):
    ...
    # Forward-fill / step sampling for discrete fields (gear, DRS)
    ...
    drs_resampled = drs_sorted[idxs].astype(float)
...
def get_quali_telemetry(session, session_type="Q"):
    ...
    try:
        if "--refresh-data" not in sys.argv:
            data = load_computed_pickle_first(
                _computed_telemetry_pkl_candidates(session, cache_suffix)
            )
            print(f"Loaded precomputed {cache_suffix} telemetry data.")
            print("The replay should begin in a new window shortly!")
    ...
    results = _fork_pool_map(_process_quali_driver, driver_args)
    for result in results:
